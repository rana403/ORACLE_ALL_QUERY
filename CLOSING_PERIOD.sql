...
SELECT  OOD.OPERATING_UNIT "OPERATING_UNIT",GET_OU_NAME_FROM_ID(OOD.OPERATING_UNIT ) OU_NAME,
...
and period_name like '%Jan%22%'  
and  PERIOD_NAME NOT LIKE '%Adj%' 
and   (SELECT name FROM gl_sets_of_books gsp where GSP.SET_OF_BOOKS_ID = GPS.SET_OF_BOOKS_ID) = '106-KSRM Power Plant Limited'
ORDER BY period_year DESC, 
period_num DESC  


SELECT NAME FROM GL_SETS_OF_BOOKS GSP order by name


select * from rcv_shipment_headers where receipt_num= 80000006 and ship_to_org_id=602





select * from RCV_TRANSACTIONS where SHIPMENT_HEADER_ID  = 1105868

select * from MTL_MATERIAL_TRANSACTIONS 

SELECT * FROM MTL_TRANSACTION_TYPES WHERE  TRANSACTION_TYPE_NAME  LIKE '%Item%Loan%Adjustment%'


select * from  ORG_ORGANIZATION_DEFINITIONS WHERE  TRANSACTION_TYPE_NAME  LIKE '%Item%Loan%Adjustment%'
