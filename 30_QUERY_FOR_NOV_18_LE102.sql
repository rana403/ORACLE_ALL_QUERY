 /* THIS PROCESS IR RUN FOR MAR-21 KSPL     DATE: 10-JUL-2021  */


/* 
 NOV-18 WAS REVISE 2 TIMES : 
 FIRST TIME CREATE TABLE NAME WAS XLA_AE_HEADERS_BCK_KSRM11 
 AND SECOND TIME TABLE NAME WAS XLA_AE_HEADERS_BCK_KSRM11V
 
 */

select * from org_organization_definitions WHERE SET_OF_BOOKS_ID= 2041  and LEGAL_ENTITY = 26280
...
WHERE transaction_date >= TO_DATE('01/11/18 00:00:00','dd/mm/yy hh24:mi:ss')
AND transaction_date <= TO_DATE('30/11/18 23:59:59','dd/mm/yy hh24:mi:ss')
AND EXISTS
(
SELECT 1
...
UPDATE gmf_rcv_accounting_txns grat
 ...
 WHERE transaction_date >=
          TO_DATE ('01/11/18 00:00:00', 'dd/mm/yy hh24:mi:ss')
       AND transaction_date <=
              TO_DATE ('30/11/18 23:59:59', 'dd/mm/yy hh24:mi:ss')
       AND EXISTS
              (SELECT 1
                 FROM mtl_parameters p, hr_organization_information hoi
                WHERE p.process_enabled_flag = 'Y'
...
UPDATE gmf_invoice_distributions
 ...
 WHERE accounted_date >= TO_DATE ('01/11/18 00:00:00', 'dd/mm/yy hh24:mi:ss')
       AND accounted_date <=
              TO_DATE ('30/11/18 23:59:59', 'dd/mm/yy hh24:mi:ss')
...
UPDATE gmf_incoming_material_layers giml
 ...
 WHERE layer_date >= TO_DATE ('01/11/18 00:00:00', 'dd/mm/yy hh24:mi:ss')
       AND layer_date <= TO_DATE ('30/11/18 23:59:59', 'dd/mm/yy hh24:mi:ss')
       AND EXISTS
              (SELECT 1
                 FROM mtl_parameters p, hr_organization_information hoi
                WHERE p.process_enabled_flag = 'Y'
...
UPDATE gme_resource_txns
   SET posted_ind = 0
 WHERE trans_date >= TO_DATE ('01/11/18 00:00:00', 'dd/mm/yy hh24:mi:ss')
       AND trans_date <= TO_DATE ('30/11/18 23:59:59', 'dd/mm/yy hh24:mi:ss')
...
CREATE TABLE xla_events_bck_ksrm11v
AS
   SELECT xe.*
...
CREATE TABLE xla_ae_headers_bck_ksrm11v
AS
   SELECT *
     FROM xla_ae_headers
    WHERE application_id = 555
          AND event_id IN (SELECT event_id FROM xla_events_bck_ksrm11v);
          
       
          
-- Query-18

CREATE TABLE xla_ae_lines_bck_ksrm11v
AS
   SELECT *
     FROM xla_ae_lines
    WHERE application_id = 555
          AND ae_header_id IN
                 (SELECT ae_header_id FROM xla_ae_headers_bck_ksrm11v);
...
CREATE TABLE xla_dist_link_bck_ksrm11v
AS
   SELECT *
...
CREATE TABLE xla_ae_seg_val_bck_ksrm11v
AS
   SELECT *
...
CREATE TABLE xla_trans_enti_upg_bck_ksrm11v
AS
   SELECT *
     FROM XLA_TRANSACTION_ENTITIES_UPG
    WHERE     application_id = 555
          AND LEGAL_ENTITY_ID = '&LE_ID'
          AND ENTITY_ID IN (SELECT ENTITY_ID FROM xla_ae_headers_bck_ksrm11v);
      ...
      WHERE application_id = 555
            AND ae_header_id IN
                   (SELECT DISTINCT ae_header_id
                      FROM xla_dist_link_bck_ksrm11v);
...
DELETE FROM xla_ae_lines
      WHERE application_id = 555
            AND ae_header_id IN
                   (SELECT DISTINCT ae_header_id FROM xla_ae_lines_bck_ksrm11v);
...
DELETE FROM xla_ae_segment_values
      WHERE ae_header_id IN
               (SELECT DISTINCT ae_header_id FROM xla_ae_seg_val_bck_ksrm11v);
...
DELETE FROM XLA_TRANSACTION_ENTITIES_UPG
      WHERE APPLICATION_ID = 555 AND LEGAL_ENTITY_ID = '&LE_ID'
            AND ENTITY_ID IN
                   (SELECT ENTITY_ID FROM xla_ae_headers_bck_ksrm11v);
...
/*  b) Reset gl_item_cst.final_flag to 0    */  
      ...
      UPDATE gl_item_cst
      SET final_flag = 0
      WHERE period_id  in (103) --- <period_id FROM query a>
      AND   final_flag = 1;
...
/*  c) Reset cm_cmpt_dtl.rollover_ind to 0  */
      ...
      UPDATE cm_cmpt_dtl
      SET rollover_ind = 0
      WHERE period_id = 103  ----<period_id FROM query a>
      AND   rollover_ind = 1;
